...
`run` and `resume` wait in-process by default (poll every 20 s, `DEFAULT_POLL_INTERVAL` at contracts.py:23); `--no-wait` returns after submission. One waiting `run` chains through `auto` and `reviewed` gates, including revisions, until a `human` gate, a blocked review, the terminal stage, or an error (workflow.py:2130-2147). `run` prints `RUN_DIR <path>` to stderr as soon as the run directory exists (workflow.py:1629); after `run` or `resume` the CLI prints `RUN <run_status> stage <stage_id> <stage_status>` and exits 2 when the run or stage ended failed, blocked, cancelled, incomplete, or with an unknown submission outcome (`_report_outcome`, run_responses_v2.py:179). Dry runs print no `RUN` line.
...
| `GateType` (line 270), `StageStatus` (line 300), `ALLOWED_STAGE_TRANSITIONS` (line 326) | Gate categories, per-stage lifecycle states (including `revision_requested`), and the legal transitions between them; dead-end outcomes and abandoned pre-submission states may re-enter `staging_inputs` for an explicit `--stage` rerun; `assert_stage_transition()` (line 354) `SystemExit`s on any other move |
| `ReviewConfig` (line 401) | `reviewer` (`codex` default, `claude`, `none`), `model`, `effort`, `timeout_seconds` (1800), `max_revisions` (1); defaults `gpt-5.6-sol`/`high` for codex and `opus`/`xhigh` for claude (lines 396-397) |
| `MODEL_CAPS` (line 183) | Capability map for durable `gpt-5.6` and explicit family variants |
| `validate_model_options()` (line 733) | `SystemExit`s on max_output_tokens overage, wrong cache retention, structured-output mismatch |
| `resolve_under_root()` (line 626) | **The single chokepoint that prevents path escape** — `SystemExit` if a path resolves outside root |

> **Insight**
> - `resolve_under_root()` (contracts.py:626-636) does `path.resolve().relative_to(root.resolve())`; the resolve step follows symlinks, so a symlink trick cannot sneak files in or out. This makes the One-Root Policy *mechanical*, not just documented.
...
- Validates stage uniqueness, ordering, and carry-forward references; `review_bundle_from_stage_id` is accepted only as a legacy alias for `handoff_from_stage_id`, and naming both with different values is rejected (lines 239-245).
...
`run_workflow()` (line 1606) is the orchestrator. Per invocation:
...
load/create run_manifest.json
        ↓
apply --handoff-note if given            (_apply_handoff_note, line 880)
apply any pending reviewed-gate verdict  (_stage_review_pending, line 725)
        ↓
_determine_next_stage (line 247)         ← respects human/blocked gates;
                                            refuses every other state with
...
│ 9. if gate=reviewed: _apply_stage_gate (line 779)   │
...
- **Dry-run renders every stage.** `run --dry-run` without `--stage` walks the whole workflow under `<run_dir>/dry_runs/`, writing `input_manifest.{json,md}`, `request_payload.json`, and the `upload_inputs/` staging directory per stage (lines 1690-1695 and 1806-1829). Handoffs from stages that have not run yet are satisfied by placeholder files under `dry_runs/stubs/<stage_id>/` (`_dry_run_stub`, line 476). This is the whole pre-launch check.
- **Token preflight.** The exact count (`POST /responses/input_tokens`, with the payload projected onto the fields that endpoint accepts, openai_client.py:36) is the only token check and fails closed when the count exceeds `max_input_tokens` or the model context window minus the requested output and a safety margin (`_token_preflight_state`, line 1016), leaving the stage `blocked_preflight`. Retries of the count back off (2^attempt seconds, capped at 30 s; line 1091). There is no local estimate. `--skip-token-count` disables the exact count.
- **Validators are advisory.** Post-output validators (`evidence_references_v1` and friends) write `validator_report.json` and set `validators_passed` / `validator_report_path` on the stage summary; a failed validator is a `WARNING`, never a block (`_run_stage_validators`, lines 1462-1506).
- **Auto-progression is gated** (lines 2137-2147): the engine only loops to the next stage when the stage completed, has a next stage, its gate is `auto` or `reviewed` (and approved), the invocation is waiting, and the operator did not pin `--stage`.
- **Crash recovery is driven by the run manifest.** Every attempt's `local_state`, `response_id`, and the `pid` of the runner process that opened it (`_record_attempt_start`, line 1349) live in `run_manifest.json`, persisted by `_persist_stage_state` (line 1389) before and after each step. `resume` (`resume_stage`, line 2209) rehydrates the stored `response_id`, polls to terminal, finalizes, and applies a pending reviewed gate; `refresh` is the same path with `refresh_status_only`, which polls and records the remote status but never finalizes. `_determine_next_stage` sorts every other state into an exact next command (`_not_runnable_message`, line 538): states with a recorded response (`RESUMABLE_STAGE_STATES`, line 76) get the `resume` command; dead ends (`DEAD_END_STAGE_STATES`, line 84: `failed_no_artifact`, `blocked_preflight`, `failed_complete`, `cancelled`, `incomplete`) and abandoned pre-submission states (`PRE_SUBMISSION_STAGE_STATES`, line 68: `staging_inputs`, `uploading`, `preflight_passed`, where no request reached the API) rerun as a new attempt only with an explicit `run --stage <id>` (`RERUNNABLE_STAGE_STATES`, line 92), a pre-submission rerun is refused while the recorded `pid` is still alive (`_refuse_if_attempt_is_live`, line 527), and a `run` without `--stage` never reruns them implicitly (lines 291-293). `submitting` and `submission_outcome_unknown` (`RECONCILE_STAGE_STATES`, line 74) are never resubmitted: an ambiguous `POST /responses` failure (lines 2011-2051) writes `submission.error.json` and sets `submission_outcome_unknown`; `run` prints dashboard reconciliation guidance (find the response by metadata `stage_id`, record its id with status `submitted` and `resume`, or set `failed_no_artifact` and rerun with `--stage`), while `resume`/`refresh` (line 2250) and `cancel` (line 2385) refuse it. A revision attempt that dies before review keeps its `revision_of_attempt_id` on rerun (lines 1713-1717) and does not count against `max_revisions` (`_revision_count`, line 500). A wait failure after submission exits with the exact `resume` command (lines 2092-2097).
...
A standard-library HTTP client (`urllib`); `jsonschema` is the one third-party dependency, used for contract validation. Retries with capped exponential backoff (30 s, line 103) apply only to safe methods (`GET`/`HEAD`/`OPTIONS`, line 25) on `{408, 409, 429, 500, 502, 503, 504}`; a `POST /responses` is never retried automatically, because a duplicate submission would be worse than an unknown outcome. `wait_for_terminal_response` (line 346) writes `response.latest.json` on every poll and rides out transient retrieve failures (`POLL_TRANSIENT_STATUS_CODES`, line 99: 408, 409, 425, 429, 5xx, plus network errors with no status) for up to `MAX_CONSECUTIVE_POLL_FAILURES` (30, line 100) consecutive polls before raising, because a background response keeps running through a polling outage.
...
`_apply_stage_gate` (workflow.py:779) runs when a `reviewed` stage reaches `completed` and its `review_status` is not already `approved`, `human_approved`, or `not_required`. The reviewer invocation and the manifest update that follows run under the run lock (workflow.py:841-871), so a second `run` on the same run directory during a review is refused instead of starting a duplicate reviewer.

1. `build_review_job` composes the bounded input: workflow/run/stage/attempt ids, the stage task text, the root-relative paths of `artifact.md`, `input_manifest.md`, and the handoff inputs (`_reviewed_handoff_paths`, workflow.py:733), and `validator_report_path` when a validator report exists. The prompt lets the reviewer open any file listed in `input_manifest.md` to spot-check a material claim, and nothing else.
...
4. `run_review` executes the command with `timeout_seconds`, writes `stdout_<stamp>.txt`, `stderr_<stamp>.txt`, and `invocation_<stamp>.json` (argv, duration, exit code, artifact SHA256, and cost/token fields when the CLI reports them), then `extract_verdict` unwraps CLI envelopes and `normalize_verdict` coerces synonyms (`approved`, `reject`, `blocked`, ...) onto the contract and validates it. A codex verdict is rejected when the transcript never opened the artifact path (`reviewer_read_artifact`, reviewer.py:150): a schema-valid verdict from a reviewer that never read the artifact is not a review.
5. On success it writes `review/verdict.json` and `review/reviewer_notes.md`. A non-zero exit, a timeout, a missing CLI, an unparseable verdict, or a rejected codex verdict raises `ReviewError`; the stage stays `completed` with its review pending and the error names the continuation (`retry_guidance`, reviewer.py:138): the same command with `--run-dir <run_dir>` retries the review, `--handoff-note <note.md>` approves the artifact in place, `--reviewer none` skips it, and a bare `run` without `--run-dir` would start a new run and resubmit the stage at full cost.
...
Back in `_apply_stage_gate` (workflow.py:852-857); "revisions so far" counts only revision attempts that reached a verdict (`REVISION_COUNTED_STATES`, workflow.py:94), so a revision that died before review is not spent:
...
When the loop re-enters a `revision_requested` stage, the new attempt records `revision_of_attempt_id` (workflow.py:1707-1717); an explicit `--stage` rerun of a revision attempt that died before review inherits the same `revision_of_attempt_id`, so the reviewer context is not lost. Two hooks change what the model sees:

- `_stage_task_text` (workflow.py:567) prefixes `REVISION_INSTRUCTIONS` to the stage task: resolve every blocking finding or argue against it with evidence, preserve untouched parts, output the full artifact.
- `_revision_handoff_entries` (workflow.py:581) attaches the prior attempt's `review/reviewer_notes.md` and `artifact.md` under **Reviewed Handoff Inputs**.

The reviewer runs again on the revised artifact with `revision_of_attempt_id` in its job.

### 4.4 Handoff to the next stage

`_gate_handoff_entries` (workflow.py:625) builds the Reviewed Handoff Inputs for a stage whose `carry_forward.handoff_from_stage_id` names an earlier stage:
...
`_apply_handoff_note` (workflow.py:880) is the human side: given `--handoff-note <path>`, it finds the latest `human` or `reviewed` stage in `waiting_for_review` without a note (failing that, a `completed` reviewed stage whose review is still pending after a reviewer failure), records `handoff_note_path`, and sets `review_status: human_approved`. `_determine_next_stage` then lets the following stage run.

`reference_context_from_stage_ids` (`_reference_context_from_stage_outputs`, workflow.py:436) is the cheaper, lower-authority carry-forward: it attaches earlier `artifact.md` files under **Reference Context** with no approval requirement.
...
9. The stage is `terminal`, so the run completes; the CLI prints `RUN completed stage <stage_id> completed` to stderr and the run manifest path to stdout.
...
If the process dies after a stage completes but before its review is applied, the next `run --run-dir <run_dir>` applies the pending review first, then continues (`resume` refuses a `completed` stage). If a stage ends as a dead end instead, the CLI exits 2 with a `WARNING` naming `run --run-dir <run_dir> --stage <id>`, which reruns that stage as a new attempt in the same run.
...
The `.local/` prefix is in `.gitignore` and `DIRECTORY_SKIP_NAMES` (contracts.py:59): these artifacts never get committed and are never re-attached as evidence to a future run. Stage state (`status`, `current_attempt_id`, each attempt's `local_state`, `response_id`, and opening `pid`) lives only in `run_manifest.json`.

---

## 7. Tests and Validation

The `automation/tests/` suite has 12 modules (113 tests): attachments, audit-gap closure, context quality, contracts, durability, eval harness, evidence-synthesis example, example pack, gstack pack, model migration, reviewed gates, workflow. Standard run:
